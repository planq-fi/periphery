...
import '@planq-fi/contracts/contracts/interfaces/callback/IPlanqFiFlashCallback.sol';
import '@planq-fi/contracts/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the PlanqFi flash function
contract PairFlash is IPlanqFiFlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `planqFiFlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IPlanqFiPool pool = IPlanqFiPool(PoolAddress.computeAddress(factory, poolKey));
